...
@videos.post('/token', summary="Create access and refresh tokens for user", response_model=TokenSchema, include_in_schema=False)
...
@videos.get('/{id}', response_model=VideoOut,include_in_schema=True)
...
@videos.get('/play/{id}', response_model=DetailsVideoOut,summary="Video play Page")
async def video_play_page(id: int):
    ...
    video_details=DetailsVideoOut(**video,likes=likes,comments=comments,dislikes=dislikes)
...
async def increase_count(id:int,object_type,total_count:int):
    print(total_count)
    if object_type=='like':
        query = db_videos.update().where(db_videos.c.id == id).values(total_likes=total_count)
    elif object_type=='dislike':
        query = db_videos.update().where(db_videos.c.id == id).values(total_dislikes=total_count)
    elif object_type=='comment':
        query = db_videos.update().where(db_videos.c.id == id).values(total_comments=total_count)
