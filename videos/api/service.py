...
async def get_dislikes(video_id):
    r=httpx.get(f"http://127.0.0.1:8003/dislikes/dislikes-by-video?video_id={video_id}")
    dislikes=r.json()
    return [item["username"] for item in dislikes] 

async def get_comments(video_id):
    r=httpx.get(f"http://127.0.0.1:8003/comments/comments-by-videos?video_id={video_id}")
    comments= r.json()
