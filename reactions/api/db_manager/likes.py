...
from  reactions.api.db import comments,dislikes,likes, database
import os
import requests
...
async def get_like_by_video_and_user(video_id,username):
    query = likes.select().where(likes.c.video_id==video_id,likes.c.username==username)
...
async def delete_like(video_id,username):
    ...
    if not video:
        raise HTTPException("Video not found")
    query = likes.delete().where(likes.c.video_id==video_id,likes.c.username==username)
    return await database.execute(query=query)
