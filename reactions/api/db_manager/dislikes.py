from reactions.api.models import CommentIn,CommentOut,DislikeIn,DislikeOut,LikeIn,LikeOut
from  reactions.api.db import comments,dislikes, database
import os
import requests
...
from reactions.api.db_manager.likes import add_like,delete_like,get_all_likes_by_video

async def get_dislike_by_video_and_user(video_id,username):
    query = dislikes.select().where(dislikes.c.video_id==video_id,dislikes.c.username==username)
...
async def delete_dislike(video_id,username):
    ...
    if not video:
        raise HTTPException("Video not found")
    query = dislikes.delete().where(dislikes.c.video_id==video_id,dislikes.c.username==username)
