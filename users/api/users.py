...
from  users.api import db_manager
from users.api.utils import get_hashed_password
...
@users.post('/login', summary="Create access and refresh tokens for user", response_model=TokenSchema)
...
@users.get('/me', summary='Get details of currently logged in user', response_model=UserOut)
async def get_me(user: SystemUser = Depends(db_manager.get_current_user)):
    return user

@users.get('/remote/me/', summary='Get details of currently logged in user in remote server', response_model=UserOut)
